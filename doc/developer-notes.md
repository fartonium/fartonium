...
Run with the -testnet option to run with "play fartoniums" on the test network, if you
...
Fartonium Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- BitcoinMiner : Generates fartoniums (if wallet is enabled).
...
However, in open source software such as Fartonium Core, where everyone uses
...
pay attention to for reviewers of Fartonium Core code.

General Fartonium Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Fartonium Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Fartonium Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Fartonium Core so that they can be integrated
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:fartonium-project/fartonium.git
